...
XML.ObjTree.prototype.parseHTTP = function ( url, options, callback ) {
    ...
    if ( typeof(HTTP) != "undefined" && HTTP.Request ) {
        myopt.uri = url;
        req = new HTTP.Request( myopt );        // JSAN
        if ( req ) trans = req.transport;
    } else if ( typeof(Ajax) != "undefined" && Ajax.Request ) {
        req = new Ajax.Request( url, myopt );   // ptorotype.js
        if ( req ) trans = req.transport;
    }
...
XML.ObjTree.prototype.parseElement = function ( elem ) {
    ...
    if ( elem.childNodes && elem.childNodes.length ) {
        ...
        if ( textonly ) {
            if ( ! retval ) retval = "";
            for ( var i3=0; i3<elem.childNodes.length; i3++ ) {
                retval += elem.childNodes[i3].nodeValue;
            }
        } else {
            if ( ! retval ) retval = {};
            for ( var i4=0; i4<elem.childNodes.length; i4++ ) {
                var key2 = elem.childNodes[i4].nodeName;
                if ( typeof(key2) != "string" ) continue;
                var val2 = this.parseElement( elem.childNodes[i4] );
                if ( ! val2 ) continue;
                if ( typeof(cnt[key2]) == "undefined" ) cnt[key2] = 0;
                cnt[key2] ++;
                this.addNode( retval, key2, cnt[key2], val2 );
            }
        }
    }
    return retval;
